...
*
Date: 11/7/14
** Description: Implimentation of bst.h. Creates and
** manages a binary search tree data structure. Allows for
** addition to, subtraction from, and search of the tree.
** Main function impliments tests of functionality.
**
** Special thanks to David Crisman for helping spot
** one well hidden syntax error
**
** Input: n/a
** Output: n/a
*
...
{	
	if(cur == NULL) //add new node here if null
	{
		struct Node *newNode;
...
{
	//create iterator node pointer to move through tree
    struct Node *currentNode;
    currentNode = tree->root;

    while(compare(currentNode->val, val) != 0)
    {
        ...
        //head left or right in tree
        if(compare(val, currentNode->val) == -1)
        {
    ...
    //if it got here, the value was found
    return 1;
}
